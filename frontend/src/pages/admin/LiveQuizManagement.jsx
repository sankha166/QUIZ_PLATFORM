...
export default function LiveQuizManagement() {
  ...
  const togglePublish = async (quiz) => {
    ...
    try {
      ...
      alert(
        error?.response?.data?.message || "Unable to change live quiz status.",
      );
    } finally {
      setBusy(null);
    }
  ...
  const remove = async (quiz) => {
    if (!window.confirm(`Delete live quiz "${quiz.title}" permanently?`))
      return;
    setBusy(quiz.id);
    try {
  ...
  const statusPill = (status) => {
    ...
    return (
      <span className={`rounded-full px-2.5 py-1 text-xs ${cls}`}>
        {status}
      </span>
    );
  };
  ...
  const controls = (quiz, status) => (
    <div className="flex flex-wrap gap-2">
      ...
      <button onClick={() => openStats(quiz)} className="btn-secondary text-xs">
        Stats
      </button>
      {status !== "Live now" && status !== "Completed" && (
        <button
    ...
    <AdminLayout>
      <div className="min-w-0 space-y-5">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="page-subtitle">
              One absolute IST schedule is shared by admin and students.
            </p>
          </div>
          <Link to="/admin/quizzes/create?live=1" className="btn-primary">
            Create live quiz
          </Link>
        </header>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {[
            ["Created", stats.total ?? items.length],
            ["Draft", stats.drafts ?? grouped.draft.length],
            ["Live now", stats.live_now ?? grouped.live.length],
            ["Upcoming", stats.upcoming ?? grouped.upcoming.length],
            ["Completed", stats.completed ?? grouped.completed.length],
          ].map(([label, value]) => (
        ...
        <section className="card overflow-hidden p-0">
          <div className="border-b border-slate-200 px-4 py-4 sm:px-5">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Countdowns update every second. Schedules are shown in IST.
            </p>
          </div>
              ...
              <tbody>
                {items.map((quiz) => {
                  ...
                  return (
                    ...
                    >
                      <td className="p-3">
                        <p className="text-slate-900">{quiz.title}</p>
                      ...
                      <td className="p-3">{statusPill(status)}</td>
                      <td className="p-3 text-slate-600">
                        {displayIst(quiz.live_start_at)}
                      </td>
                      <td className="p-3">
                        {status === "Upcoming" ? (
                          <span className="text-indigo-700">
                            {countdown(quiz.live_start_at, now)}
                          </span>
                        ) : status === "Live now" ? (
                          <span className="text-rose-700">
                            {remaining(quiz.computed_end_at, now)} left
                          </span>
                        ) : (
                      ...
                      <td className="p-3">{controls(quiz, status)}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {selected && (
          <div
            className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-3"
            onClick={() => setSelected(null)}
          >
            <div
              className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white p-5 shadow-2xl sm:p-7"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between gap-3">
                <div>
                  <p className="eyebrow text-rose-600">Live quiz</p>
                  <h2 className="mt-1 text-xl">{selected.title}</h2>
                  <p className="text-sm text-slate-500">Event statistics</p>
                </div>
              ...
              <div className="mt-5 space-y-1 text-sm text-slate-600">
                <p>Start: {displayIst(selected.live_start_at)}</p>
                <p>Automatic end: {displayIst(selected.computed_end_at)}</p>
              </div>
            </div>
          </div>
        )}
