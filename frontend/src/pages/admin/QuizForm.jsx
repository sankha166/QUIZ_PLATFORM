...
const parseIstSchedule = (value) => {
  ...
  return {
    ...
    hour: parts.hour,
    minute: parts.minute,
    period: parts.dayPeriod?.toUpperCase() || "AM",
  };
};
...
export default function QuizForm() {
  ...
  useEffect(() => {
    setValue("live_start_at", toIstIso(schedule));
  }, [schedule, setValue]);
  ...
  const onSubmit = async (form) => {
    ...
    try {
      ...
      if (isLive && !liveStart)
        return setError("Select a valid start date, time and AM/PM in IST.");
              ...
              <label className="flex items-center gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-4">
                ...
                <span>
                  ...
                  <span className="mt-1 block text-xs text-slate-500">
                    No category is required. The event appears only in Live
                    Quizzes.
                  </span>
                </span>
              </label>
              ...
              <>
                <div className="rounded-3xl border border-rose-200 bg-gradient-to-br from-rose-50 to-orange-50 p-5">
                  ...
                  <p className="mt-1 text-xs text-slate-600">
                    Configure the rating multiplier used for each correct
                    answer.
                  </p>
                ...
                <div className="rounded-3xl border border-indigo-200 bg-indigo-50/60 p-5">
                  ...
                  <p className="mt-1 text-xs text-slate-600">
                    Set one timer once and automatically apply it to every live
                    question.
                  </p>
                  <label className="mt-4 flex items-start gap-3">
                    ...
                    />
                    <span>
                      <b>Same time for every question</b>
                      <span className="mt-1 block text-xs text-slate-500">
                        Recommended. Updating this value also updates all
                  ...
                  <div className="mt-4 max-w-sm">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {sameQuestionTime
                        ? "Applied automatically to all questions."
                        : "Individual timers can be adjusted in Manage Questions."}
                    </p>
                  </div>
                </div>
                <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <h3 className="text-lg font-bold">Live Start Time</h3>
                      <p className="mt-1 text-xs text-slate-500">
                        Indian Standard Time (IST, UTC+05:30). This is the
                        single event time used by admin and students.
                      </p>
                    </div>
                  ...
                  <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          value={schedule.hour}
                          onChange={(e) =>
                            setSchedule((s) => ({ ...s, hour: e.target.value }))
                          }
                        >
                    ...
                    <div className="rounded-2xl bg-slate-50 p-3">
                      ...
                      <p className="mt-1 font-semibold text-slate-800">
                        {schedule.date
                          ? `${schedule.date} · ${schedule.hour}:${schedule.minute} ${schedule.period}`
                          : "Choose date and time"}
                      </p>
                      <p className="mt-1 text-[11px] text-slate-500">
                        No automatic extra 5h 30m is added.
                      </p>
                    </div>
                  </div>
                  <input type="hidden" {...register("live_start_at")} />
                </div>
              </>
            )}
            <div>
