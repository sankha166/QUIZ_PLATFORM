...
export default function QuizForm() {
  const { id } = useParams(),
    ...
    isEdit = Boolean(id),
    createLive = new URLSearchParams(location.search).get("live") === "1";
  const [categories, setCategories] = useState([]),
    [domains, setDomains] = useState([]),
    [domainId, setDomainId] = useState(""),
    [loading, setLoading] = useState(isEdit),
  ...
  const {
    ...
    defaultValues: {
      ...
      live_same_question_time: true,
      live_question_time_seconds: 30,
      ...DEFAULT_SCORING,
    },
  });
  const categoryId = watch("category_id"),
    isLive = watch("is_live_quiz"),
    allDomains = watch("live_all_domains"),
    sameQuestionTime = watch("live_same_question_time");
  useEffect(() => {
    Promise.all([
      getDomains(),
      getCategories(),
      ...
      .then(([dr, cr, qr]) => {
        const ds = dr.data.domains || [],
          cs = cr.data.categories || [];
        setDomains(ds);
        setCategories(cs);
        if (qr?.data?.quiz) {
          const q = qr.data.quiz,
            cat = cs.find((c) => String(c.id) === String(q.category_id));
          setDomainId(
            q.live_all_domains
              ? "all"
              : String(q.domain_id ?? cat?.domain_id ?? ""),
          );
          reset({
            ...q,
            ...
            live_question_time_seconds: Number(
              q.live_question_time_seconds ?? 30,
            ),
  ...
  const onSubmit = async (form) => {
    ...
    try {
      ...
      if (
        isLive &&
        (!form.live_question_time_seconds ||
          Number(form.live_question_time_seconds) < 5 ||
          Number(form.live_question_time_seconds) > 600)
      )
        return setError(
          "Live question time must be between 5 and 600 seconds.",
        );
      const selected = domainCategories.find(
      ...
      const data = {
        ...
        live_question_time_seconds: Math.min(
          600,
          Math.max(5, Number(form.live_question_time_seconds) || 30),
        ),
      };
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setDomainId(v);
                    setValue("category_id", "");
                    setValue("live_all_domains", v === "all");
                  }}
                >
                ...
                <p className="mt-1 text-xs text-slate-500">
                  {domainId === "all"
                    ...
                    : "Students in the selected domain can see this event."}
                </p>
              </div>
              <div>
                ...
                <select
                  ...
                  {...register("category_id", {
                    required: isLive && !allDomains,
                  })}
                  disabled={
                    !domainId || (!allDomains && !domainCategories.length)
                  }
                >
                  <option value="">
                    {allDomains
              ...
              <label className="flex items-center gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-4">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setValue("live_all_domains", on);
                    setDomainId(
                      on ? "all" : domains[0]?.id ? String(domains[0].id) : "",
                    );
                    setValue("category_id", "");
                  }}
                />
                <span>
                  ...
                  <span className="block text-xs text-slate-500 mt-1">
                    No category is required. This event is shown to every
                    student only in Live Quizzes.
                  </span>
                </span>
              </label>
            )}
            ...
            {isLive && (
              <>
                <div className="rounded-3xl border border-rose-200 bg-gradient-to-br from-rose-50 to-orange-50 p-5">
                  <h3 className="font-bold text-lg">🔴 Live Scoring Rules</h3>
                  <p className="text-xs text-slate-600 mt-1">
                    Configure the speed-based rating multipliers used for this
                    event.
                  </p>
                  <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-3">
                    {[
                      ["live_score_025", "≤ 0.25× time", "2.0×"],
                      ["live_score_050", "≤ 0.50× time", "1.5×"],
                      ["live_score_075", "≤ 0.75× time", "1.25×"],
                      ["live_score_100", "≤ 1.00× time", "1.0×"],
                      ["live_score_wrong", "Wrong answer", "-0.5×"],
                  ...
                  <p className="mt-3 text-xs text-slate-500">
                    Unanswered = 0×. Final rating = question marks × multiplier.
                  </p>
                </div>
                <div className="rounded-3xl border border-indigo-200 bg-indigo-50/60 p-5">
                  <h3 className="font-bold text-lg">⏱ Question Timing</h3>
                  <p className="mt-1 text-xs text-slate-600">
                    Use one timer for every question instead of entering it
                    repeatedly.
                  </p>
                  <label className="mt-4 flex items-start gap-3">
                    <input
                      ...
                      {...register("live_same_question_time")}
                    />
                    <span>
                      <b>Use the same time for every live question</b>
                      <span className="block text-xs text-slate-500 mt-1">
                        When enabled, the backend automatically applies this
                        value to every current and future live question.
                      </span>
                    </span>
                  </label>
                  <div className="mt-4 max-w-sm">
                    ...
                    <input
                      ...
                      max="600"
                      className="input"
                      {...register("live_question_time_seconds", {
                        required: true,
                        valueAsNumber: true,
                        min: 5,
                        max: 600,
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      5–600 seconds.{" "}
                      {sameQuestionTime
                        ? "This timer is used automatically for all live questions."
                        : "Individual timers can be set in Manage Questions."}
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  <input
                    type="datetime-local"
                    className="input"
                    {...register("live_start_at", { required: true })}
                  />
                  <p className="mt-1 text-xs text-slate-500">
                    Starts automatically at this IST time. No end time is
                    required; the event ends after its questions.
                  </p>
                </div>
              </>
            )}
