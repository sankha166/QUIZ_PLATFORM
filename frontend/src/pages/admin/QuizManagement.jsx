...
export default function QuizManagement() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [quizResponse, categoryResponse, domainResponse] =
        await Promise.all([
          getQuizzes({
  ...
  useEffect(() => {
    fetchData();
  }, [search, statusFilter, domainFilter, categoryFilter]);
  ...
  const visibleCategories = useMemo(
    () =>
      domainFilter === "all"
        ? categories
        : categories.filter(
            (c) => String(c.domain_id) === String(domainFilter),
          ),
      ...
      quizzes
        .map((q) => ({
          ...
          resolved_domain_name:
            q.domain_name ?? q.resolved_domain_name ?? "Unknown",
        }))
        .sort((a, b) => {
          const domainCompare = (a.resolved_domain_name || "").localeCompare(
            b.resolved_domain_name || "",
          );
          if (domainCompare !== 0) return domainCompare;
          const categoryCompare = (a.category_name || "").localeCompare(
            b.category_name || "",
          );
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2 min-w-[230px]">
                          ...
                          <button
                            ...
                            className={`rounded-lg px-2.5 py-1.5 text-xs ${q.status === "published" ? "bg-yellow-50 text-yellow-700 hover:bg-yellow-100" : "bg-green-50 text-green-700 hover:bg-green-100"}`}
                          >
