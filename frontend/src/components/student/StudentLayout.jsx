          ...
          <div className="flex min-h-16 items-center gap-2 sm:gap-4">
            ...
            <Link to="/student/dashboard" className="flex min-w-0 shrink-0 items-center gap-2 sm:gap-3">
              ...
              <div className="hidden min-w-0 xs:block sm:block">
                <p className="truncate text-sm font-semibold text-slate-900">Quizora</p>
                <p className="hidden text-xs text-slate-500 sm:block">Student Portal</p>
              </div>
            </Link>
            ...
            <div className="hidden min-w-0 flex-1 items-center justify-center gap-1 md:flex lg:gap-2">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`whitespace-nowrap rounded-xl px-3 py-2 text-sm font-medium transition lg:px-4 ${isActive(item.path) ? 'bg-brand-50 text-brand-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                >
                  {item.label}
                </Link>
              ))}
            ...
            <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-3">
              ...
              <div className="relative" ref={profileRef}>
                <button
                  ...
                  className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-white text-slate-700 shadow-sm sm:h-11 sm:w-11"
                ...
                {profileOpen && (
                  <div className="absolute right-0 top-full mt-2 w-[min(18rem,calc(100vw-1.5rem))] rounded-2xl border border-slate-200 bg-white p-3 shadow-xl">
                    <div className="border-b border-slate-100 px-3 pb-3">
                      <p className="truncate text-sm font-semibold text-slate-900">{user?.name}</p>
                      <p className="truncate text-xs text-slate-500">{user?.email}</p>
                    </div>
                    <Link to="/student/profile#profile-settings" className="mt-2 block rounded-xl px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50">Update profile</Link>
                    <button type="button" onClick={handleLogout} className="mt-1 w-full rounded-xl bg-slate-900 px-3 py-2.5 text-left text-sm font-semibold text-white hover:bg-slate-800">Logout</button>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mx-auto max-w-7xl space-y-1 px-3 py-3 sm:px-6">
              {navItems.map((item) => (
                <Link
                  ...
                  className={`block rounded-xl px-4 py-3 text-sm font-semibold ${isActive(item.path) ? 'bg-brand-50 text-brand-700' : 'text-slate-700 hover:bg-slate-50'}`}
                >
                  {item.label}
                </Link>
              ))}
              <Link to="/student/profile" onClick={() => setMobileOpen(false)} className="block rounded-xl px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">My Profile</Link>
              <div className="mt-2 flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-slate-900">{user?.name}</p>
                  <p className="truncate text-xs text-slate-500">{user?.email}</p>
                </div>
                <button type="button" onClick={handleLogout} className="shrink-0 rounded-xl bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-800">Logout</button>
              </div>
            </div>
          </div>
        )}
