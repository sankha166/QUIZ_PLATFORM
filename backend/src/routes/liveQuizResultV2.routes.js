...
router.get('/:quizId', authenticate, async (req,res,next) => {
  try {
    const quiz = await query(`SELECT q.id,q.title,q.status,q.live_start_at,q.live_end_at,COUNT(DISTINCT qs.id)::int question_count
      FROM quizzes q LEFT JOIN questions qs ON qs.quiz_id=q.id
      WHERE q.id=$1 AND q.is_live_quiz=true GROUP BY q.id`, [req.params.quizId]);
    ...
    const event = await query(`SELECT COUNT(*)::int attempts,COUNT(DISTINCT user_id)::int participants,
    ...
    if (req.user.role === 'STUDENT') {
      const attempt = await query(`SELECT id,COALESCE(live_rating,0)::numeric(10,2) rating
        ...
        ORDER BY completed_at DESC NULLS LAST,id DESC LIMIT 1`, [q.id,req.user.id]);
      if (attempt.rows.length) {
        ...
        const wrong = Number(c.wrong || 0);
        const unanswered = Math.max(0, Number(q.question_count || 0) - answered);
        const rank = await query(`WITH scores AS (
          SELECT user_id,MAX(live_rating) rating FROM attempts
          WHERE quiz_id=$1 AND status!='in_progress' GROUP BY user_id
        ) SELECT COUNT(*)::int+1 rank FROM scores WHERE rating>$2`, [q.id,a.rating]);
        mine = { attempted:true, rating:Number(a.rating||0), rank:Number(rank.rows[0]?.rank||1), answered, correct, wrong, unanswered };
      }
    }

    res.json({success:true,result:{
      id:q.id,title:q.title,status:q.status,live_start_at:q.live_start_at,live_end_at:q.live_end_at,
      question_count:Number(q.question_count||0),
      attempts:Number(event.rows[0]?.attempts||0),
      students:Number(event.rows[0]?.participants||0),
      participants:Number(event.rows[0]?.participants||0),
      avg_rating:Number(event.rows[0]?.avg_rating||0),
      ...mine
    }});
  } catch (e) { next(e); }
});
