...
const getProfile = async (userId) => {
  try {
    ...
    // Keep the profile page usable if optional analytics/domain tables are not yet migrated.
    if (err.status === 404) throw err;
    const basic = await query(
      `SELECT id, name, email, role, status, avatar_url, bio, created_at
       FROM users WHERE id = $1`,
      [userId]
    );
    if (!basic.rows.length) { const e = new Error('User not found'); e.status = 404; throw e; }
    return {
      ...basic.rows[0],
      preferred_domain_id: null,
      preferred_domain_name: null,
      quizzes_attempted: 0,
      average_score: 0,
      highest_score: 0,
      quizzes_passed: 0,
